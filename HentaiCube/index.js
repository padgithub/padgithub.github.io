...
const DOMAIN = 'https://hentaicb.xyz/';
const method = 'GET';
...
class HentaiCube extends paperback_extensions_common_1.Source {
    ...
    getHomePageSections(sectionCallback) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let obj of $('.item__wrap ', '.slider__container .slider__item').toArray()) {
                ...
                let id = (_a = $(`.slider__thumb a`, obj).attr('href')) !== null && _a !== void 0 ? _a : title;
            ...
            for (let obj of $('.popular-item-wrap', '#manga-recent-3 .widget-content').toArray()) {
                let title = $(`.popular-content a`, obj).text().trim();
                const image = $(`.popular-img > a > img`, obj).attr('data-src') ? $(`.popular-img > a > img`, obj).attr('data-src').replace('-75x106', '') : $(`.popular-img > a > img`, obj).attr('src').replace('-75x106', '');
                let id = (_c = $(`.popular-img > a`, obj).attr('href')) !== null && _c !== void 0 ? _c : title;
            ...
            url = '';
            request = createRequestObject({
                url: 'https://hentaicb.xyz/?s&post_type=wp-manga&m_orderby=latest',
                method: "GET",
            });
            let newUpdatedItems = [];
            data = yield this.requestManager.schedule(request, 1);
            ...
            for (let obj of $('.c-tabs-item__content', '.tab-content-wrap').toArray()) {
                ...
                const image = (_d = $('.c-image-hover > a > img', obj).attr('data-src')) !== null && _d !== void 0 ? _d : $('.c-image-hover > a > img', obj).attr('src');
                let id = (_e = $(`.c-image-hover > a`, obj).attr('href')) !== null && _e !== void 0 ? _e : title;
                newUpdatedItems.push(createMangaTile({
                    id: id !== null && id !== void 0 ? id : "",
                    image: encodeURI(image),
                    title: createIconText({
                        text: (_f = HentaiCubeParser_1.decodeHTMLEntity(title)) !== null && _f !== void 0 ? _f : "",
                    }),
                    subtitleText: createIconText({
                        text: subtitle
                    }),
            ...
            url = DOMAIN;
            request = createRequestObject({
                url: 'https://hentaicb.xyz/?s&post_type=wp-manga&m_orderby=views',
                method: "GET",
            });
            let newAddItems = [];
            data = yield this.requestManager.schedule(request, 1);
            ...
            url = '';
            request = createRequestObject({
                url: 'https://hentaicb.xyz/?s&post_type=wp-manga&m_orderby=new-manga',
                method: "GET",
            });
            let newItems = [];
            data = yield this.requestManager.schedule(request, 1);
    ...
    getViewMoreItems(homepageSectionId, metadata) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (homepageSectionId) {
                case "new":
                    url = `https://hentaicb.xyz/page/${page}/?s&post_type=wp-manga&m_orderby=new-manga`;
                    url2 = `https://hentaicb.xyz/page/${page + 1}/?s&post_type=wp-manga&m_orderby=new-manga`;
                    select = 0;
                    break;
                case "new_updated":
                    url = `https://hentaicb.xyz/page/${page}/?s&post_type=wp-manga&m_orderby=latest`;
                    url2 = `https://hentaicb.xyz/page/${page + 1}/?s&post_type=wp-manga&m_orderby=latest`;
                    select = 1;
                    break;
                case "view":
                    url = `https://hentaicb.xyz/page/${page}/?s&post_type=wp-manga&m_orderby=views`;
                    url2 = `https://hentaicb.xyz/page/${page + 1}/?s&post_type=wp-manga&m_orderby=views`;
                    select = 2;
                    break;
                default:
    ...
    getSearchResults(query, metadata) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (year.length !== 0) { //year + sort
                if (tags[0].split('.')[0] === 'year' || tags[0].split('.')[0] === 'sort') {
                    if (year.length !== 0 && sort.length !== 0) {
                        ...
                        if (tags[0].split('.')[0] === 'year') {
                            ...
                            set = 1;
                            url = encodeURI(`https://hentaicb.xyz/page/${page}/?s&post_type=wp-manga&${sort[0]}`);
                        }
                    }
                }
            }
                ...
                set = 1;
                url = encodeURI(`https://hentaicb.xyz/page/${page}/?s=${(_d = query.title) !== null && _d !== void 0 ? _d : ""}&post_type=wp-manga&${convertGenres(genre)}&op=&author=&artist=&release=&adult=&${convertStatus(status)}&${sort[0]}`);
            }
            const request = createRequestObject({
                url,
                method: "GET"
    ...
    getSearchTags() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const tags4 = [];
            const counts = [];
            let url = `https://hentaicb.xyz/?s=&post_type=wp-manga`;
            let request = createRequestObject({
                url: url,
                method: "GET",
            });
            ...
            let $ = this.cheerio.load(response.data);
            let url2 = `https://hentaicb.xyz/manga/`;
...
},{"./HentaiCubeParser":57,"paperback-extensions-common":12}],57:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
